...
<?php foreach($this->fields as $i => $field): ?>
        <div class="form-group">
            <div class="col-md-3 adminer-edit-column-left">
                <input name="fields[<?php echo $i ?>][field]" class="form-control" value="<?php
                    echo $field['field'] ?>" data-maxlength="64" autocapitalize="off">
                <input type="hidden" name="fields[<?php echo $i ?>][orig]" value="<?php echo $field['field'] ?>">
            </div>
            <div class="col-md-2 adminer-edit-column-middle">
                <select name="fields[<?php echo $i ?>][type]" class="form-control">
...
<?php endforeach ?>
            ...
            <div class="col-md-1 adminer-edit-column-middle">
                <input name="fields[<?php echo $i ?>][length]" class="form-control<?php
                    if($field['_length_required_']): ?> required<?php endif ?>" value="<?php
                    echo $field['length'] ?>" size="3">
            </div>
            <div class="col-md-2 adminer-edit-column-middle">
                <select name="fields[<?php echo $i ?>][collation]" class="form-control<?php
...
<?php if($this->unsigned): ?>
                <select name="fields[<?php echo $i ?>][unsigned]" class="form-control<?php
...
<?php if(isset($field['on_update'])): ?>
                <select name="fields[<?php echo $i ?>][on_update]" class="form-control<?php
...
<?php if($this->foreign_keys): ?>
                <select name="fields[<?php echo $i ?>][on_delete]" class="form-control<?php
...
<?php endforeach ?>
            ...
            <div class="col-md-1 adminer-edit-column-null" style="padding-left:10px;padding-right:1px;">
                <input type="checkbox" name="fields[<?php echo $i ?>][null]" value="1" <?php
                    if($field['null']): ?>checked <?php endif ?>/>
            </div>
            <div class="col-md-1 adminer-edit-column-middle">
                <input type="radio" name="auto_increment_col" value="<?php echo $i ?>" <?php
                    if($field['auto_increment']): ?>checked <?php endif ?>/>
            </div>
            <div class="col-md-2 adminer-edit-column-right" data-name="<?php
                echo $field['field'] ?>" data-id="<?php echo $i ?>">
                <button type="button" class="btn btn-primary btn-xs" id="adminer-table-add-column">
                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                </button>
